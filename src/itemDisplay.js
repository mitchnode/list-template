...
export function itemDisplay(item) {
  ...
  if (
    item.getDueDate() <= format(Date.now(), "dd/MM/yyyy") &&
    !item.getFlag()
  ) {
  ...
  if (!item.getEdit()) {
    ...
    itemDOMFlag.addEventListener("click", () => {
      item.toggleFlag();
      refresh(content, projectViewDisplay(item.getProject()));
    });
    ...
    itemDOMDelete.addEventListener("click", () => {
      item.deleteItem();
      refresh(content, projectViewDisplay(item.getProject()));
    });
    ...
    if (item.getEdit()) {
      ...
      itemDOMEdit.addEventListener("click", (e) => {
        ...
        item.saveItem(
          itemDOMName.value,
          itemDOMDesc.value,
          itemDOMPriority.value,
          itemDOMDueDate.value
        );
        refresh(content, projectViewDisplay(item.getProject()));
      });
    } else {
      itemDOMEdit.className = "edit";
      itemDOMEdit.addEventListener("click", () => {
        item.editItem();
        refresh(content, projectViewDisplay(item.getProject()));
      });
    }
